...
async function scrapeGoogleResult(keyword, domain, numberOfPages) {
  ...
  const pageWaitDurationRangeStart = 5000;
  const pageWaitDurationRangeEnd = 9000;
  ...
  page.setUserAgent(randomUserAgents[parseInt(random(0, 8))]);
  await page.goto('https://www.google.com/ncr');
  await page.goto(domain);

  page.focus('input[title="Search"]');

  await page.type(keyword, { delay: random(100, 500) });
  ...
  for (var i = index + 1; i <= numberOfPages ; i++) {
    ...
    await page.waitFor(random(start, end));
